...
npm install
npx expo start
Scan QR code with Expo Go (Android) or Camera app (iOS). Press W to open in browser.
...
Configuration
FitnessApp/src/config/api.js is already pointed to the live Railway backend. No additional configuration needed.

Features
User registration and login with Firebase Authentication
5-step onboarding with goal setting
Workout logging with history
Meal logging with USDA food search and automatic macro calculation
Mood tracking with trend chart
Mindfulness exercises with live timer
AI-powered recommendations via Python rule engine (21 rules)
Weekly and monthly progress reports
Motivational messages based on mood data
GDPR compliant — consent stored, full account deletion available
Firestore security rules — complete user data isolation
